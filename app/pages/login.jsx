...
class Login extends React.Component {
  ...
  tabSelect(index) {
    if ((this.state.isRegistering === true && index !== 0) || (this.state.isRegistering === false && index !== 1)) {
      this.setState({
      ...
      <FormContainer>
        ...
        <form onSubmit={this.onSubmit.bind(this)}>
          <Flexer>
            <InputContainer>
              {
                this.state.isRegistering
                  ? (<input id="name" type="text" name="name" placeholder="Name (e.g. Hacker McHackerface)" value={this.state.name} onChange={this.handleAttributeChange.bind(this)}/>)
                  : (undefined)
              }
              <input id="email" type="email" name="email" placeholder="Email (e.g. hacker@umich.edu)" value={this.state.email} onChange={this.handleAttributeChange.bind(this)}/> {
                !this.state.isResetting
                  ? (<input id="password" type="password" name="password" placeholder="Password (e.g. hunter2)" value={this.state.password} onChange={this.handleAttributeChange.bind(this)}/>)
                  : (undefined)
              }

              {
                !this.state.isRegistering && !this.state.isResetting
                  ? (<ResetContainer>
                    <ResetLink href="#" onClick={this.resetSelect}>
            ...
            <ButtonGroup>
              <RoundedButton type="submit" color={this.props.theme.primary}>
                Confirm
              </RoundedButton>
            </ButtonGroup>
            {
...
function mapStateToProps(state) {
  return {userState: state.userState, theme: state.theme.data};
}
